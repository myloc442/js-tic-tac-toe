...
export function checkGameStatus(cellValues) {
  ...
  const winSetIndex = winSetIndexList.findIndex((set) => {
  ...
  // WIN
  if (winSetIndex >= 0) {
    const indexValueWin = winSetIndexList[winSetIndex][0];
    const valueWin = cellValues[indexValueWin];
    return {
      status: valueWin === CELL_VALUE.CIRCLE ? GAME_STATUS.O_WIN : GAME_STATUS.X_WIN,
      winPositions: winSetIndexList[winSetIndex],
    };
  }
  // END GAME
  // PLAYING
  const isEnding = cellValues.filter((x) => x === '').length === 0;
  return {
    status: isEnding ? GAME_STATUS.ENDED : GAME_STATUS.PLAYING,
    winPositions: [],
  };
}
